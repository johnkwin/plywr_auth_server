document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.user-list-item input').forEach(input => {
        input.addEventListener('focus', function () {
            this.style.width = `${this.scrollWidth}px`; // Adjust width to fit content
            this.style.transition = 'width 0.2s ease'; // Smooth transition
        });
    
        input.addEventListener('input', function () {
            this.style.width = `${this.scrollWidth}px`; // Dynamically adjust width as user types
        });
    
        input.addEventListener('blur', function () {
            this.style.width = ''; // Reset width to default when focus is lost
        });
    });
});
